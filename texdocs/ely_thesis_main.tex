...
% custom template specific to ODU thesis.
\usepackage{ODUthesis}

%This package causes the first line of a section to be indented as required by 
...
\member{Drew Landman}  %This command produces a signature line for the 
%specified member on the
\member{Colin Britcher}  %title page. Use on instance of this command for 
%each member of the
                   %committee other than the advisor up to a total of 5 members.

\degrees{Bachelor of Science in Mechanical Engineering}

\dept{Aerospace Engineering}          %for example \dept{physics}
...
\include{docs/intro/piv_fundamentals}
...
\input{docs/experiment_setup/piv_uncertainty}
