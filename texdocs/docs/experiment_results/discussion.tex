...
in a given fluid flow. The turbulent viscosity approximation, in the notation 
of \cite{pope2000}, can be described by
...
of strain. Because the vortex model proposed by Ash, Zuckerwar, and Zardkahan 
in \cite{ash2011} is an exact solution the Reynolds averaged Navier-Stokes 
equations, and very accurately predicts the azimuthal velocity profile of an 
axial wake vortex, it was of interest to verify the validity of the turbulent 
viscosity hypothesis with experimental data. All terms in the summation of this 
equation were calculated from the PIV dataset, with the exception of gradients 
in the axial direction. Neglecting those terms, the turbulent viscosity 
assumption could was examined for each vortex dataset. The turbulent 
viscosity calculation involves division by spatial derivatives, which can 
locally be very close to zero. In area between 0.5 and 1.5 core radii where the 
velocity gradients were consistently high, that turbulent viscosity calculation 
was subject to much less noise than areas far from the core with more gentle 
gradient as shown in figure \ref{fig:run_55_turb_visc_vs_r_meshscatter}. Though 
the scatter plot is noisy, 80\% of values consistently fell within a band of 
small positive values between 0 and 0.1, an expected range of a reasonable 
constant turbulent viscosity value. These trends were observed consistently in 
all vortices.
