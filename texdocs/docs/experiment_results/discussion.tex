...
The wind tunnel flow is driven by a direct current electric motor, and the 
speed is controlled by voltage. No evidence has been found to suggest these 
findings have been influenced by the utility frequency, which is also 50 or 
60Hz.
...
\ref{eq:etap_nut} was found to be unstable, become increasingly unstable 
in the vicinity of the vortex core as shown in Figure 
\ref{fig:run_55_turb_visc_total_vs_r_meshscatter}. 

\input{figs/run_55/run_55_turb_visc_total_vs_r_meshscatter}

The velocity gradient term in the denominator from Equation \ref{eq:etap_nut}, 
shown for one test in Figure 
\ref{fig:run_55_turb_visc_vel_grad_vs_r_meshscatter} can be 
alternately represented by combining the radial and azimuthal conservation of 
momentum equations
...
\begin{equation}
\eta_P \frac{v_\theta}{r} \frac{dP}{dr} = 
\mu \Big[ \frac{d^2 v_\theta}{dr^2} + \frac{d}{dr} \frac{v_\theta}{r}\Big]
\end{equation}

\noindent
to yield a non-linear ordinary differential equation:

\begin{equation}
\rho \frac{\eta_P}{\mu} \frac{v_{\theta}^3}{r^2} = 
\Big[ \frac{d^2 v_\theta}{dr^2} + \frac{d}{dr} \frac{v_\theta}{r}\Big]
\label{eq:momentum_vel_grad}
\end{equation}

\noindent
The left hand side of Equation \ref{eq:momentum_vel_grad} contains no spatial 
derivatives and is much less susceptible to experimental noise as shown in 
figure \ref{fig:run_55_momentum_vel_grad_vs_r_meshscatter} \cite{ash2011}. With 
both sides of Equation \ref{eq:momentum_vel_grad} directly evaluated from 
experimental data, it was found to be especially valid in the core region. 
However, the steady decline beyond 2 core radii that is seen in the evaluation 
of the left hand side, it is not seen in that of the right hand side.

\input{figs/run_55/run_55_momentum_vel_grad_vs_r_meshscatter}
...
values calculated from experimental velocity gradients and Reynolds stresses 
was many times larger than expected. Insight 
