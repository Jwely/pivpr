...
def example_plots(experiment_id):
    """
    Final function used for to build plots for use in example section of final thesis document.
    """

    fig_dir = "../autofigs/examples"
    ...
    av.stream_plot(title="Stream Plot Colored by In-plane Velocities",
                   outpath=os.path.join(fig_dir, "example_stream.png"))
    ...
    av.contour_plot('R', titles="Stable Radial Velocity (R)",
                    outpath=os.path.join(fig_dir, "example_R_contour.png"))

    av.contour_plot('W', titles="Stable Axial Velocity (W)",
                    outpath=os.path.join(fig_dir, "example_W_contour.png"))

    av.contour_plot('rr', titles="Radial Fluctuation Squared (rr)",
                    outpath=os.path.join(fig_dir, "example_rr_contour.png"))

    av.contour_plot('tt', titles="Tangential Fluctuation Squared (tt)",
                    outpath=os.path.join(fig_dir, "example_tt_contour.png"))

    av.contour_plot('ww', titles="Axial Fluctuation Squared (ww)",
                    outpath=os.path.join(fig_dir, "example_ww_contour.png"))

    av.contour_plot('rt', titles="Radial/Tangential Reynolds Stress (rt)",
                    outpath=os.path.join(fig_dir, "example_rt_contour.png"))
    ...
    av.contour_plot('ctke', titles="Turbulent Kinetic Energy (k)",
                    outpath=os.path.join(fig_dir, "example_ctke_contour.png"))
    ...
    av.scatter_plot('r_mesh', 'T', 'ctke', cmap=cm.jet,
                    ...
                    yrange=(0, 6.5),
                    tight=True,
                    outpath=os.path.join(fig_dir, "example_TscatterTKE.png"))
